...
mla <- 'f1 =~ holis1 + holis2 + holis3 + holis4 + holis5
        f2 =~ social1 + social3 + social2 + social4
...
```{r, echo=TRUE}
dfitems <- select(dfenc, holis1:econ3)
mla <- 'f1 =~ holis1 + holis2 + holis3 + holis4 + holis5 + social3
        f2 =~ social1 + social2 + social4
        f3 =~ econ1 + econ2 + econ3'
threef_dwls <- cfa(mla, data=dfitems, ordered=colnames(dfitems), estimator="DWLS")
summary(threef_dwls, fit.measures=TRUE, standardized=TRUE)
# Check modification indices to identify potential model improvements
modindices(threef_dwls, sort = TRUE, minimum.value = 10)
```
