...
cat("Overall alpha =", psych::alpha(cor_poly$rho)$total$raw_alpha, "\n")
cat("Holistic alpha =", psych::alpha(corh_poly$rho)$total$raw_alpha, "\n")
cat("Econ alpha =", psych::alpha(core_poly$rho)$total$raw_alpha, "\n")
cat("Social alpha =", psych::alpha(cors_poly$rho)$total$raw_alpha, "\n")
...
KMO(cor_matrix)
cat("Overall MSA =", KMO(cor_matrix)$MSA, "\n")
cor_poly <- polychoric(select(dfenc, holis1:econ3))
KMO(cor_poly$rho)
cat("Overall MSA =", KMO(cor_poly$rho)$MSA, "\n")
```
...
```{r echo=FALSE}
# Save dfencr dataset to a csv file
data_file_path_out <- here::here("data", "Eco_Intel2_regression.csv")
write.csv(dfencr, file = data_file_path_out, row.names = FALSE)
```
...
```{r echo=TRUE}
# Run ANOVA analysis to compare program
anova_holis <- aov(holis ~ program, data=dfencr)
summary(anova_holis)
anova_social <- aov(social ~ program, data=dfencr)
summary(anova_social)
anova_econ <- aov(econ ~ program, data=dfencr)
summary(anova_econ)
anova_change <- aov(change ~ program, data=dfencr)
